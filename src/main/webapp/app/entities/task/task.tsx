...
export const Task = (props: ITaskProps) => {
  ...
  return (
    <div>
      <h2 id="task-heading">
        <Translate contentKey="codeRevealsApplicationApp.task.home.title">Tasks</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="codeRevealsApplicationApp.task.home.createLabel">Create new Task</Translate>
        </Link>
      </h2>
      <div className="table-responsive">
        {taskList && taskList.length > 0 ? (
            ...
            <tbody>
              {taskList.map((task, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{task.name}</td>
                  <td>{task.challenge ? <Link to={`challenge/${task.challenge.id}`}>{task.challenge.id}</Link> : ''}</td>
          ...
          <div className="alert alert-warning">
            <Translate contentKey="codeRevealsApplicationApp.task.home.notFound">No Tasks found</Translate>
          </div>
        )}
      </div>
    </div>
